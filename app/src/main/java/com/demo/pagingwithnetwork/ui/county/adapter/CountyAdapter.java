package com.demo.pagingwithnetwork.ui.county.adapter;
...
public class CountyAdapter extends BaseAdapter<County, ItemBaseBinding> {

    private static DiffUtil.ItemCallback<County> diffCallback = new DiffUtil.ItemCallback<County>() {
        @Override
        public boolean areItemsTheSame(@NonNull County oldItem, @NonNull County newItem) {
            return oldItem.getCountyId() == newItem.getCountyId();
        }
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder<ItemBaseBinding> holder, int position) {
        if (getItem(position) != null) holder.getBinding().itemText.setText(getItem(position).getCountyName());
    }
}
